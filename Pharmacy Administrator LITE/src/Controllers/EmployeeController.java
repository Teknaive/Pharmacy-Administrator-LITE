        ...
        if (isUserAuthorized()) { // Check if user has the necessary role
            ...
            for (Map.Entry<Integer, Employee> entry : employees.entrySet()) {
                ...
                if (!value.getEmployeeRol().equals("Owner") && value.getEmployeeID() != employeeActions.getCurrentEmployee().getEmployeeID()) {
                    row[0] = value.getEmployeeID(); // Employee ID
                    row[1] = value.getEmployeeName(); // Employee Name
                    row[2] = value.getEmployeeUser(); // Employee Username
                    row[3] = value.getEmployeeAddress(); // Employee Address
                    row[4] = value.getEmployeeEmail(); // Employee Email
                    row[5] = value.getEmployeeRol(); // Employee Role
                    row[6] = value.getEmployeePassword(); // Employee Password
